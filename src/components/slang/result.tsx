'use server'
import { getWord } from "@/services/word.firebase.service";
import { SearchResult } from "../result";
import Link from "next/link";

async function getData(word: string) {
    try{
      const wordRes = getWord(word.toLowerCase().trim().replace(/[^a-zA-Z0-9\s]/g, ''));
      return wordRes;
    }catch(err){
      console.log(err);
    }
...
export async function ResultSection(props:any){
    try{
        const word = await getData(props.slangQuery);
        return (
            <>
                <div className="p-4" id="result">
                    <SearchResult data={word}/>
                </div>
            </>
        );
    }catch(err){
        return (
            <>
                <div id='result' className="p-4 justify-center text-center">
                    ...
                    <div className=" tracking-widest mt-4">
                        <span className="text-gray-500 text-6xl block"><span>N O T F O U N D</span></span>
                        <span className="text-gray-500 text-xl">You can always contribute this word to GenZ Dictionary xD</span><br />
                        <span className="text-xl">
                            Join our <Link className="text-primary" href={"https://discord.gg/E7vZarD8j8"} target="_blank">Discord server</Link> to request and get featured on GenZ Dictinary
                        </span>
                    </div>
                </div>
            </>
